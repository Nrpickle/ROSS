...
const double Kp = .00746; // proportional - reacting to current error
...
double straightResistance = 800; //ohms - rod straight (no tension) (Daywa Custom Design (Dark Grey) - 800)
const double stopResistance = 0; //ohms - position of rod to send stop command (getting too slack)
double bentResistance = 720; //ohms - rod bent (full tension)
double desiredResistance = 0; //ohms - resistance to attempt to maintain at all times
...
void loop() {
  ...
  switch(state){
    ...
    case CONTROL_WINCH:
      ...
      if(header == 0xCA){ //calibration min
        avgvalue = 0;
        while(avgvalue == 0){
          for(int i = 0; i < 50; i++){
        ...
        if(bentResistance > 0){
          desiredResistance =  straightResistance - ((straightResistance - bentResistance) * .95); 
      ...
      if(header == 0xCB){ //calibration min
        avgvalue = 0;
        while(avgvalue == 0){
          for(int i = 0; i < 50; i++){
            myStats.add(analogRead(resistancePin));          
        ...
        bentResistance = avgvalue - 1; //Remove the extra 1 value
         if(straightResistance > 0){
          desiredResistance =  straightResistance - ((straightResistance - bentResistance) * .95); //Calculates our tension value we want. It will be 95% of the difference between our max and min values that were calibrated     
           }
        #ifdef DEBUG
...
void changeSpeed(uint8_t newSpeed, uint8_t newDir){
  ...
  if(newDir == DOWN)
    ramp = RAMP_TIME_DOWN; // The ramp is the time to get things accelerated
 ...
 #ifdef DOPID
  if(pid){ //Run PID loop only if set 

  if(winch.currentSpeed <=110){ //kickstart winch before PID happens
         //If that's the case, then we want to initialize the acceleration
         if(timerstart == true){//check to see if we need to start a timer
         t0 = millis();
       ...
       winch.currentSpeed = (double)winch.prevSpeed + (double)speedDifference*.5*(1-cos((pi*(double)deltaT)/ramp));
       timerstart = false; //We should have started the timer if we're here
  }
  
  else{
  
    // maintain resistance going down only
    if(newDir == DOWN) {
          //Read the rod resistance (0 - 1023) and run the PID function
          resistance = analogRead(resistancePin);
          rodPID.Compute();  // outputs a new speedTry
          speedTry = constrain(speedTry, -10, 10);
          // this should work in both up and down directions
          winch.currentSpeed = speedTry + winch.currentSpeed;
          winch.currentSpeed = constrain(winch.currentSpeed, 100, 160);
          speedOut = winch.currentSpeed;
          #ifdef DEBUG
  ...
  if(newDir == winch.currentDir && newSpeed == winch.currentSpeed){  //If the command is to continue moving the same speed and direction...
    ...
    return;   //...return without altering speed
     ...
     //when going up, ignore resistance and simply maintain speed
     //if(newDir == UP) {
 ...
 winch.currentSpeed = constrain(winch.currentSpeed, 0, 200); //Do not write above or below the maximum pulse widths
...
void sendStatus(){
  ...
  Serial1.print(" Res ");
  Serial1.println(resistance);
  
  Serial.print("Rev ");
  Serial.print(revsFromSurface);
  Serial.print(" Speed: ");
  Serial.println(winch.currentSpeed);
...
//    }
     if((winchEncoder.read() < depth)){
       ...
       //Serial.println("I'M SETTING THE SPEED TO DOWN AND AT SPEED_OUT");
       pid = true;
       returned = false;
     }
     else if(winchEncoder.read()>= depth){
       changeSpeed(0, STOP);
       Serial.println("I'VE REACHED THE END OF THE PROFILE, SET THE SPEED TO 0 AND STOP");
       pid = false;
       depthReached = true;
       returned = false;
     }
