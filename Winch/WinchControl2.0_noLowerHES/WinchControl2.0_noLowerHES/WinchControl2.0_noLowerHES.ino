...
//    }
    ...
    else if(winchEncoder.read() > REV(SLOW_DIST)){ //change back to else if
      changeSpeed(speedIn, UP);
      Serial.println("Going speedIn in the UP direction"); //Debug speed in
      returned = false;
    }
    else if(winchEncoder.read() <= REV(-500)){ //Winch will stop if line snaps and can't engage sensors
      changeSpeed(0, STOP);
      returned = true;
    } 
    else{
