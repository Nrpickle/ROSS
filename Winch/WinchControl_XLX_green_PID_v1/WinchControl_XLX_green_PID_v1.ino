//Winch control version for use with Mamba XL X ESC
...
//Note: Requires the PID library version 1.01 from Brett Beauregard
//Note: Requires the Timer library (Timer-master) version 1.3 by J. Christensen

#define DEBUG //Uncomment to print debugging information via serial
//#define DOPID //Uncomment to implement the PID loop
//#define CALIBRATE //Uncomment to perform calibration - NOT YET IMPLEMENTED
//#define TUNING //Uncomment to tune PID loop - NOT YET IMPLEMENTED
...
//Define digital remote start/stop pins
...
#define upLED 15
//Define analog pins
#define resistancePin 0 //fishing rod strain gauge resistance

//Define PID tuning parameters (must be >= 0)
//error = desired - input
//output = Kp*error + integral(Ki*error) - Kd*(input - lastinput)
const double Kp = 1; // proportional - reacting to current error
const double Ki = 0; // integral - reacting to error over time
const double Kd = 0; // derivative - reacting to change in error
const int KDir = REVERSE;  // if DIRECT, +INPUT leads to +OUTPUT, if REVERSE, +INPUT leads to -OUTPUT

// resistance calibration settings for the fishing rod
const double minResistance = 16924; //ohms - rod straight (no tension)
const double stopResistance = 19181; //ohms - position of rod to send stop command (getting too slack)
const double maxResistance = 28315; //ohms - rod bent (full tension)
double desiredResistance = 28000; //ohms - resistance to attempt to maintain at all times
double resistance = 28315; //ohms - measured resistance (set to max to initialize)
...
void setup() {
  ...
  statusTimer.every(statusFrequency, sendStatus); //Send a status message every second
  
  ESC.attach(9, MAX_REVERSE, MAX_FORWARD); //Connect ESC with maximum and minimum pulse width values
...
void changeSpeed(uint8_t newSpeed, uint8_t newDir){
  ...
  //Run PID loop only if set at top of code
  #ifdef DOPID
       ...
       if(newDir == DOWN) {
          ...
          winch.currentSpeed = speedTry;  
          #ifdef DEBUG
            Serial1.print("[Resistance: ");
            Serial1.print(resistance);
            Serial1.print(" Try speed: ");
            Serial1.print(speedTry);
            Serial1.println("]");
          #endif
       }
  ...
  // calculate the desired ESC pulses and write to the ESC
  constrain(winch.currentSpeed, 0, 200); //Do not write above or below the maximum pulse widths
  uint16_t speedToWrite = map(winch.currentSpeed, 0, 200, MAX_REVERSE, MAX_FORWARD); //Convert to pulse width
...
void checkResistance(){
  ...
  //Check the strain gauge resistance 
  //If the resistance is too low, the CTD may have hit the bottom
  //In that case, stop the profile and bring back to the surface
