...
const double Kp = .0746; // proportional - reacting to current error
...
void changeSpeed(uint8_t newSpeed, uint8_t newDir){
  ...
  //Run PID loop only if set at top of code
    ...
    if(newDir == DOWN) {
          ...
          winch.currentSpeed = speedTry + winch.currentSpeed; //update the speed to be adjusted based on the error
          #ifdef DEBUG
     ...
     //if(newDir == UP) {
       ...
       if(newDir == STOP) //special case, stop condition
       {
