...
double Kp = .035; // proportional - reacting to current error
double Ki = 0; // integral - reacting to error over time
...
int timerActivate = 0; //Using this to piggyback off the timer for the status message

int warningCounter = 0; //used to tell if the rod has lost tension for a certain amount of time

int avgvalue = 0; //used for the stats values to average the resistance for min/max values
...
void setup() {
  ...
  statusTimer.every(statusFrequency, sendStatus, sendWarning); //Send a status message every X milliseconds and send a warning if the rod has been straight for too long
...
void loop() {
  ...
  switch(state){
    ...
    case CONTROL_WINCH:
      if(header == 0xCD) //Set PID values using speedOut and speedIn (parameters[1-2]
      {
        Kp = speedOut/100;
        Ki = speedIn/100;
        header = 0;
